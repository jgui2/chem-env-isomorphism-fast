...
import similarity_fxn.similarity as sim
import similarity_fxn.utility as utility
...
def check_detached(contcar_path):
...
def make_graph(atoms, r=2):
    ...
    full, egos = process_atoms(atoms, nl, adsorbate_atoms=adsorbate_atoms,
    ...
    for ego in egos:
        ...
        for node, data in ego.nodes(data=True):
            mapping[node] = node
            data['element'] = node.split(":")[0]

        G = nx.relabel_nodes(ego, mapping, copy=True)
...
def get_unique_conf(confs_path='all_sites/*',energy=False):
    ...
    groups = unique_chem_envs(chem_envs, list(zip(energies, filenames)))
...
parser = argparse.ArgumentParser(description="process the DFT results. 1st Do isomorphism, 2nd remove detached configurations.")
...
### Remove the configurations where adsorbates are detached from the surface.
### Check the detached configurations by counting surface atoms (metal atoms)
### in the ego graph with r=1. 
### If any metal atom exist in the ego graphs, the configuration is good,
### otherwise, the configuration is a detached configuration, and it will be removed.
...
detached_conf_arr_i = [check_detached(contcar_path) 
                     for contcar_path in tqdm(isomorph_contcar_path_arr)]

detached_conf_arr = [i for i in detached_conf_arr_i if i is not None]
print('detached CONTCARs:',detached_conf_arr)

detached_outcar_arr = [i for i in contcar_outcar_dict 
                       if contcar_outcar_dict[i] in detached_conf_arr]
print('detached OUTCARs:',detached_outcar_arr)

for outcar_path in detached_outcar_arr:
